...
describe Decidim::SpamSignal::Conditions::AllowedTldsCommand do
  ...
  context "with forbidden tlds=(.ch,.com)" do
    ...
    context "when it is fine" do
      ...
      it("'http://www.good.url.ch'") { expect(run_condition("http://www.good.url.ch")).to be :ok }
      it("'http://infos.com.br'") { expect(run_condition("http://infos.com.br")).to be :ok }
    end

    context "when it found a forbidden domain" do
      it("'https://good.url.gov'") { expect(run_condition("http:s//good.url.gov")).to be :not_allowed_tlds_found }
      it("'Check https://crypto.finance'") { expect(run_condition("Check https://crypto.finance")).to be :not_allowed_tlds_found }
      it("'Check http://crypto.finance") { expect(run_condition("Check http://crypto.finance")).to be :not_allowed_tlds_found }
      it("'Click on [Google](https://escorts.ru) to read my blogpost'") { expect(run_condition("Click on [Google](https://escorts.ru) to read my blogpost")).to be :not_allowed_tlds_found }
      it("'Can't resist: https://a.subdomain.ru") { expect(run_condition("Can't resist: https://a.subdomain.ru")).to be :not_allowed_tlds_found }
    end
  end
  ...
  context "with empty forbidden tlds" do
    ...
    context "when it is fine" do
      ...
      it("'https://ransomware.us let's talk") { expect(run_condition("https://ransomware.us let's talk")).to be :ok }
    end
  end
end
